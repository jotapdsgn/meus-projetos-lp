/**
 * RC ESTÉTICA AUTOMOTIVA - JAVASCRIPT INTERATIVO & CONVERSÃO
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 5. Formulário de Orçamento Rápido no WhatsApp
  ...
  if (quoteForm) {
    quoteForm.addEventListener('submit', (e) => {
      ...
      const levaTraz = document.getElementById('leva-traz-select').value;
      ...
      const message = `Olá, equipe RC Estética Automotiva! Meu nome é *${clientName}*.\n\n` +
                      `Gostaria de agendar / solicitar um orçamento para o meu veículo:\n` +
                      `🚗 *Veículo/Modelo:* ${carModel || 'Não informado'}\n` +
                      `✨ *Serviço de Interesse:* ${serviceType}\n` +
                      `📍 *Precisa de Leva e Traz?* ${levaTraz}\n\n` +
                      `Quais os próximos horários disponíveis na agenda? Obrigado!`;

      const encodedMessage = encodeURIComponent(message);
      const whatsappUrl = `https://wa.me/5537999657655?text=${encodedMessage}`;
